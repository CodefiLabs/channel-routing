...
# conductor-stop.sh — Graceful shutdown of a child session
# Usage: conductor-stop.sh <slug>

SLUG="${1:?Usage: conductor-stop.sh <slug>}"
...
echo "{\"event\":\"suspend\",\"slug\":\"${SLUG}\",\"reason\":\"manual_stop\",\"ts\":\"${TS}\"}" >> "$HOME/.conductor/manifest.jsonl"
