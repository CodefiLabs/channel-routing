...
2. Check if the session is suspended by reading `~/.conductor/manifest.jsonl`:
   - Find the last event for this slug
   - If suspended, auto-resume it first using `bash ${CLAUDE_PLUGIN_ROOT}/scripts/conductor-resume.sh "<slug>" "<chat_id>"`
   - Wait 3 seconds for Claude to start

3. Route the message:
   ```
   bash ${CLAUDE_PLUGIN_ROOT}/scripts/conductor-route.sh "<slug>" "<message>"
   ```
